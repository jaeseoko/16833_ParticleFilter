...
def visualize_timestep(X_bar, tstep, output_path,laserX,laserY,x_t1,x_t0):
    # print("X_bar shape : ", np.shape(X_bar))
    x_locs = X_bar[:, 0] / 10.0
    y_locs = X_bar[:, 1] / 10.0
    ...
    LX = laserX
    LY = laserY
    # visualize_myLaser(laserX,laserY,x_t1)
    if(LX[0] != 0):    
        laser = plt.scatter(LX,LY,c='b', marker = '*')

    
    # # laser = plt.plot([x_t1[0]/10*np.ones_like(laserX),LX],[x_t1[1]/10*np.ones_like(laserX),LY],'b-')
...
if __name__ == '__main__':
    ...
    # print("occupancy map size and shape = ",np.size(occupancy_map), " , ", np.shape(occupancy_map))
    # print(occupancy_map)
    ...
    for time_idx, line in enumerate(logfile):
        ...
        sum = 0
        # for m in tqdm(range(0, num_particles)):
        for m in range(0, num_particles):
            ...
            yInt = int(x_t1[0]/10.0)

            # if (not (0 <= occupancy_map[yInt, xInt] <= 0.0) and meas_type=='L') or (occupancy_map[yInt, xInt] == -1):
            if occupancy_map[yInt, xInt] ==1.0 :
                # print("CHECKING: \n")
                # print(occupancy_map[yInt,xInt])
                w_t = 0
                probs = 0
            ...
            if (meas_type == "L"):
                ...
                '''
                CHECKING PROBABILITIES OF PARTICLES            
                '''
                sum+=w_t    
        ...
        if args.visualize and num_particles > 1:
            visualize_timestep(X_bar, time_idx, args.output,sensor_model.laserX,sensor_model.laserY,x_t1,x_t0)
